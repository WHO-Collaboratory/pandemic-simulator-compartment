...
2. The two pieces of code I always have to write by hand are the schema declaration and the ODE `equation()`. Initialization usually just calls `super().__init__(config)` and tacks on model-specific fields.
...
| Need | File |
|---|---|
| Base class, helpers, `_compute_equations`, `_apply_flow`, `_apply_interventions`, `_prepare_demographic_state` | [compartment/model.py](../compartment/model.py) |
...
- **Sex-structured wildlife host with multi-rate FOI / harmonic births / density-dependent deaths (manual flows, hand-declared `*_total`)**: [hantavirus_jax_model/model.py](../compartment/models/hantavirus_jax_model/model.py)
...
- **Never** call `datasets.load()` at import time, inside `equation()`, or in the parallel worker fan-out. Load once, stash on `self`.
...
- **Getting data locally:** run `python -m compartment.datasets login` first (Cognito/WHO SSO — **no AWS credentials, api key, or configuration**; the target is a constant). Then `pull <slug> --version <v>` populates the cache, `list` shows what's available, and `push --wait` publishes a new immutable version and blocks for the scan verdict. A cache miss errors with the exact `pull` command.
- **One catalog, all environments.** Datasets live in a single owning environment; every other environment reads the same bucket cross-account and resolves pins against the same registry. There is no per-environment catalog and nothing to promote — so a model can never reach an environment whose dataset is missing. Don't add a second datasets module or a second scan Lambda; the single owner is what keeps `status = PUBLISHED` trustworthy.
...
- Renaming a compartment? Search the codebase for the old ID — variant subclasses (`variants.py`), `COMPARTMENT_DELTA_GROUPING`, and any manual indexing in `equation()` will all need updating in lockstep.
