...
- **`Disease.travel_sigma`** (float, optional): Percentage of each admin zone's population away from home on a given day, causing mixing across admin zones (0-100; `0` disables travel). Mobility is model-owned, so this lives inside the `Disease` block alongside the model's other parameters, and only models that declare it accept it — see [docs/guides/mobility.md](docs/guides/mobility.md). Some models declare extra mobility parameters next to it (mpox: `travel_scale_km`; hantavirus_human: `travel_alpha`).
